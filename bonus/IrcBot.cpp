...
void IrcBot::startListening()
{
    ...
    while (true)
    {
        ...
        if (incoming[0] == ':') {
            size_t bangPos = incoming.find("!");
            size_t spacePos = incoming.find(" ");
            if (bangPos != std::string::npos && bangPos < spacePos) {
                sender = incoming.substr(1, bangPos - 1);
            }
        }

        else if (incoming.find("PRIVMSG") != std::string::npos && incoming.find("/help") != std::string::npos)
        {
